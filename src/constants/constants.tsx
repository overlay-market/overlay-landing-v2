export const GENERAL_LINKS = {
    GOVERNANCE: 'https://commonwealth.im/overlay',
    GITHUB: 'https://github.com/overlay-market',
    DOCUMENTATION: 'https://docs.overlay.market/',
    COMMUNITY_DOCS:
      'https://flash-asp-362.notion.site/Overlay-Protocol-29da60fd11e9436ea69e2a5e0db28a74',
    WHITEPAPER_V1:
      'https://gateway.pinata.cloud/ipfs/QmVMX7DH8Kh22kxMyDFGUJcw1a3irNPvyZBtAogkyJYJEv',
    DISCORD: 'https://discord.gg/FgNhRNtQYv',
    TWITTER: 'https://twitter.com/OverlayProtocol',
    MIRROR: 'https://mirror.xyz/0x7999C7f0b9f2259434b7aD130bBe36723a49E14e',
    PRIVACY_POLICY: '',
    TERMS_OF_SERVICE: 'https://overlay.market/#/tos',
    LAUNCH_APP: 'https://app.overlay.market',
    CLAIM: 'https://app.overlay.market/claim',
    SPEARBIT_AUDIT: 'https://github.com/overlay-market/v1-core/blob/main/audits/spearbit/audit.pdf',
    LEAST_AUTHORITY_AUDIT: 'https://github.com/overlay-market/v1-core/blob/main/audits/leastauthority/audit.pdf',
    TRAIL_OF_BITS_AUDIT: 'https://github.com/overlay-market/v1-core/blob/main/audits/trailofbits/audit.pdf',
    INMUNEFI_BOUNTY: 'https://immunefi.com/bounty/overlay/',
    OVERLAY_APP: 'https://app.overlay.market', 
  }